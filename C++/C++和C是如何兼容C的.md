...
**结论：采用g++编译完成后，函数的名字将会被修饰，编译器将函数的参数类型信息添加到修改后的名字中，因此当相同函数名的函数拥有不用类型的参数时，在g++编译器看来是不同的函数。**

对于C程序，函数是未加任何修饰的，而对于C++程序，函数名是会被参数类型信息修饰的，这就造成了它们之间无法直接相互调用。
...
通过上面的分析，我们知道extern "C"的真实目的是实现**类C和C++的混合编程**，那么首先要处理的问题就是要让我们所写的C++程序和C程序函数的**编译时的修饰规则**和**链接时的修饰规则**保持一致。
...
既然C语言的静态库只能按照C的规则去编译这些函数，那么我们只要让C++程序按照C语言的链接规则去找到函数名不就解决了？
...
```c
#ifdef __cplusplus//如果是c++程序，就执行extern “C”
extern "C"{
#endif
#include"..\..\Stack_C\Stack_C\stack.h"
#ifdef __cplusplus
...
这样同样的一段代码，无论是C++程序还是C程序就都能去调用该静态库了。
...
但这样问题也随之而来，如果是C++的程序去调用这个库，就无法使用C++的特性函数重载了。



**总结：C++和C之间的混合编译，为了消除函数名修饰规则不同的的差别，我们需要使用extern ”C“来改变C++的编译或者连接方式。**
