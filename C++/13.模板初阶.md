...
> 2. **对于非模板函数和同名函数模板，如果其他条件（参数类型）都相同，在调动时会优先调用非模板函数而不会从该模板产生出一个实例**。如果模板可以产生一个具有更好匹配的函数， 那么将选择模板。
>
> ```cpp
...
> 非类型形参，就是用**一个常量**作为类(函数)模板的一个参数，在类(函数)模板中可将该参数当成常量来使用。
