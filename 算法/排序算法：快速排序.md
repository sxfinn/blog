...
![img](https://cdn.jsdelivr.net/gh/sxfinn/Pic/img/202204221121115.png)
...
2. **循环交换：** 当 left = right 时跳出；
   1. **指针 left** 遇到比基准值小的值则执行 `left++` 跳过，直到找到大于基准的值；
   2. **指针 right** 遇到比基准值大的值则执行 `right--` 跳过，直到找到小于基准的值；
...
![](https://cdn.jsdelivr.net/gh/sxfinn/Pic/img/202204221057061.gif)
...
![](https://cdn.jsdelivr.net/gh/sxfinn/Pic/img/202204221522869.gif)
...
![](https://cdn.jsdelivr.net/gh/sxfinn/Pic/img/202204221552304.gif)
...
![image-20220422162927369](https://cdn.jsdelivr.net/gh/sxfinn/Pic/img/202204221629445.png)

如图所示，随着递归**深度的增加**，栈帧的开辟个数也逐层递增，最终会变成只拥有一个元素或者零个元素的子问题，而对于这些一个元素的序列甚至零个元素的序列，也开辟了一块栈帧，而这是完全没有必要的，栈区的资源非常有限，这样做实在浪费，因此我们在处理这种**较少规模**的问题时，可以采用数据量少时整体性能比较好的**插入排序**作为小规模问题的补充。
