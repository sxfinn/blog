...
在程序还没执行到test函数时，只要进入了该函数的函数体，static修饰的 i 就已经为1了，就像是一进来这个块，i 就已经被创建了，而不是等到 程序走到 `static int i=1`时才被创建，事实上赋值语句从来没有执行过。
（如果静态变量没有初始化，则默认为0.）
...
* **static修饰全局变量**
  作用：改变了全局变量的作用域，让静态的全局变量只能在自己的源文件内使用，而无法在本源文件外部使用。
  全局变量**作用域是整个工程**，被static修饰后作用域就只是此源文件内部了。
